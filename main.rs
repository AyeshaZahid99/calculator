...
fn main() {
	...
	println!("SELECT\n1: ➕  2: ➖   3: ➗   4: ✖️  5: ❗️  6: ^2  7:^3   8:a^n  9:1/n  10:sqrt(inv)    11:sqrt   12:% ");
...
println!("the answer is {}",multiplication);
	...
	let number:u8=number.trim().parse().unwrap();
    let mut x=1;
    for i in 1..number{
         x=x+i*x;
    }
...
else if inp_1==options[12]{
let mut b=String::new();
	println!("Enter the integer");
	io::stdin().read_line(& mut b).expect("incorrect entry");
	let b:f64=b.trim().parse().unwrap();
let percentage:f64=b/100.0;
println!("The answer is {}",percentage);
}
